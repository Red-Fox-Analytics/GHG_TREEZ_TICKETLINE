WITH ticketline_cte AS (
    ...
    SELECT   PRODUCTBRAND, PRODUCTNAME, PRODUCTTYPE ,STOREID , OPEN_DATE  
    		, sum (NETSALES) NETSALES 
    FROM {{source('FIVETRAN_DATABASE','ticketline')}}--FIVETRAN_DATABASE.TREEZ_FIVETRAN.TICKETLINE ----
    GROUP BY  PRODUCTBRAND, PRODUCTNAME, PRODUCTTYPE , STOREID , OPEN_DATE 
    ORDER BY  PRODUCTBRAND, PRODUCTNAME, PRODUCTTYPE ,STOREID , OPEN_DATE   
), inventory_cte AS (    
...
left JOIN ticketline_cte tc
	ON ic.PRODUCTBRAND= tc.PRODUCTBRAND AND ic.PRODUCTNAME=tc.PRODUCTNAME AND  ic.PRODUCTTYPE=tc.PRODUCTTYPE
 	AND ic.STOREID=tc.STOREID AND ic.START_DATE=tc.OPEN_DATE 