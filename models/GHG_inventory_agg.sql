WITH ticketline_cte AS (
    ...
    SELECT   PRODUCTBRAND, PRODUCTNAME, PRODUCTTYPE ,STOREID , DATECLOSED  
			...
			, sum(qty) UNIT_SALES_TICKETLINE_RF
			, (sum(UNIT_SALES_TICKETLINE_RF) over(PARTITION BY  PRODUCTBRAND, PRODUCTNAME, PRODUCTTYPE ,STOREID 
											ORDER BY  PRODUCTBRAND, PRODUCTNAME, PRODUCTTYPE ,STOREID , DATECLOSED desc ROWS  between 28 preceding and current ROW))/4 AVG_WEEKLY_UNIT_SALES_ROLLING_4W_RF
...
(
	 SELECT   PRODUCTBRAND, PRODUCTNAME, PRODUCTTYPE,PRODUCTSUBTYPE ,STOREID , DATECLOSED ,  "SIZE",  UNITOFMEASURE
	 FROM {{source('FIVETRAN_DATABASE','ticketline')}}--FIVETRAN_DATABASE.TREEZ_FIVETRAN.TICKETLINE
	 GROUP BY  PRODUCTBRAND, PRODUCTNAME, PRODUCTTYPE ,PRODUCTSUBTYPE, STOREID , DATECLOSED	,  "SIZE",  UNITOFMEASURE 
), maxdate_cte AS 
(
	SELECT max(START_DATE) MAXDATE_RF FROM inventory_cte
)
SELECT ic.PRODUCTBRAND, ic.PRODUCTNAME, ic.PRODUCTTYPE,tsub.PRODUCTSUBTYPE,tsub."SIZE", tsub.UNITOFMEASURE,  cast(ic.STOREID as varchar) STOREID, ic.START_DATE,
		...
		, tc.UNIT_SALES_TICKETLINE_RF, tc.NETSALES_TICKETLINE_RF, tc.SALESTAXAMOUN_TTICKETLINE_RF
		, tc.EXCISETAXAMOUNT_TICKETLINE_RF 
		, AVG_WEEKLY_UNIT_SALES_ROLLING_4W_RF
