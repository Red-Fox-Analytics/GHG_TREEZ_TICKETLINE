...
(
	...
	FROM TICKETLINE 
	WHERE CAST(DATECLOSED AS date) BETWEEN dateadd(day, -84, (SELECT MAX_DATE FROM maxdate_cte)) AND (SELECT MAX_DATE FROM maxdate_cte) 
	--ORDER BY CAST(DATECLOSED AS date)
	...
	FROM TICKETLINE 
	WHERE CAST(DATECLOSED AS date) BETWEEN dateadd(day, -24*7, (SELECT MAX_DATE FROM maxdate_cte)) AND (SELECT MAX_DATE FROM maxdate_cte) 
	--ORDER BY CAST(DATECLOSED AS date)
	...
	FROM TICKETLINE 
	WHERE CAST(DATECLOSED AS date) BETWEEN dateadd(day, -52*7, (SELECT MAX_DATE FROM maxdate_cte)) AND (SELECT MAX_DATE FROM maxdate_cte) 
	--ORDER BY CAST(DATECLOSED AS date)
	UNION ALL 
	
	SELECT *, (SELECT MAX_DATE FROM maxdate_cte) MAX_DATE,'104 Weeks' AS TIME_PERIOD 
	FROM TICKETLINE 
	WHERE CAST(DATECLOSED AS date) BETWEEN dateadd(day, -104*7, (SELECT MAX_DATE FROM maxdate_cte)) AND (SELECT MAX_DATE FROM maxdate_cte) 
